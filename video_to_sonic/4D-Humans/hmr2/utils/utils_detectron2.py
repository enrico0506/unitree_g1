...
class DefaultPredictor_Lazy:
    ...
    def __init__(self, cfg):
        ...
        """
        self.device = self._infer_device(cfg)
    ...
    @staticmethod
    def _infer_device(cfg) -> torch.device:
        """Pick the device requested by config, or fall back safely."""
        if isinstance(cfg, CfgNode):
            requested = getattr(getattr(cfg, "MODEL", object()), "DEVICE", None)
        else:
            requested = (
                OmegaConf.select(cfg, "runtime_device", default=None)
                or OmegaConf.select(cfg, "model.device", default=None)
                or OmegaConf.select(cfg, "train.device", default=None)
            )

        requested = str(requested or "cuda").lower()
