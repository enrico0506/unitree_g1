...
#   INFER_HZ=3                              # each inference pegs the GPU ~206ms; lower = smoother
#                                           # video when the overlay is on (GPU is shared with the camera)
#   OWL_THRESHOLD=0.12                      # OWL scores run low (~0.1-0.4); raise to cut noise
#   OWL_NMS_IOU=0.5                         # class-agnostic NMS to dedupe OWL's overlapping boxes
...
INFER_HZ="${INFER_HZ:-3}"
OWL_THRESHOLD="${OWL_THRESHOLD:-0.12}"
