...
The dashboard's object detector (`DETECTOR_IMPL=nanoowlsam`, behind the
`detector.py` seam). It runs **NanoOWL** (TensorRT OWL-ViT) for open-vocabulary
labeled boxes, dedupes them with class-agnostic NMS, then **NanoSAM** (TensorRT
ResNet18 encoder + MobileSAM decoder) segments **each box into a mask** — so you get
*both* labels *and* full per-object coverage. The dashboard draws the masks as
translucent polygons under the boxes (the "Object Detection" overlay).
...
| `OWL_THRESHOLD` | `0.12` | Raise (→0.18) to cut false positives, lower for recall. OWL scores run ~0.1–0.4. |
| `OWL_NMS_IOU` | `0.5` | Class-agnostic NMS to dedupe OWL's overlapping boxes. Lower (→0.4) merges more. |
| `INFER_HZ` | `3` | Each inference pegs the GPU ~206 ms. **Lower this if the video lags** when the overlay is on (the GPU is shared with the camera). ~2 is smoother, ~5 is the max. |
...
## Too many / noisy detections (e.g. "everything is a table")

OWL-ViT is open-vocab and over-fires low-confidence boxes on generic surfaces, and has
no built-in NMS. Two knobs, both in `run_detect_nanoowlsam.sh`:
- **`OWL_THRESHOLD`** (default 0.15) — raise toward 0.20–0.25 to drop weak detections.
- **`OWL_NMS_IOU`** (default 0.5) — class-agnostic NMS already collapses overlapping
  duplicates; lower it (e.g. 0.4) to merge more aggressively.
- Trim `DETECT_PROMPTS` — dropping broad classes like `table` removes a common false-positive.
